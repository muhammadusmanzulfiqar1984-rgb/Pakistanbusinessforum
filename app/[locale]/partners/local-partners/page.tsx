  ...
  return (
    <>
      ...
      <main>
              ...
              {localPartners.map((p) => (
                <Link key={p.slug} href={`/partners/local-partners/${p.slug}`} className="card local-partner-card">
                  <div className="local-partner-logo">
                    <Image src={p.favicon} alt={`${p.title} favicon`} className="local-partner-favicon" width={64} height={64} />
                  </div>
        ...
        <LogoSlider variant="light" label="International Partners" logos={policyLogos} />
      </main>
      <Footer />
    </>
  )
